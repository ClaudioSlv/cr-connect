...
export async function POST(request: Request) {
  ...
  if (!response.ok) { console.error("Diagnostic provider error", response.status, await response.text()); return Response.json({ error: "Não foi possível concluir a consulta agora. Tente novamente em alguns instantes." }, { status: 502 }); }
