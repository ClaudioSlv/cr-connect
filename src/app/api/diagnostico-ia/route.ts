...
export async function POST(request: Request) {
  ...
  const prompt = mode === "technical" ? `Você é um assistente de biblioteca técnica para oficina automotiva no Brasil. Veículo: ${vehicle || "não informado"}. Pergunta: ${symptom}. Responda em português, de forma objetiva: procedimento geral, pontos a conferir, dados que devem ser confirmados no manual técnico do veículo e alertas de segurança. Não invente torque, fluido ou especificação; quando o dado depender de versão, informe que deve ser confirmado no manual.` : `Você é um assistente de triagem para oficina automotiva no Brasil. Veículo: ${vehicle || "não informado"}. Sintoma relatado: ${symptom}. Responda em português, de forma objetiva, com: 1) causas prováveis em ordem, 2) verificações seguras iniciais, 3) quando não dirigir/usar guincho. Não afirme certeza, não instrua procedimentos perigosos e recomende avaliação de profissional.`;
