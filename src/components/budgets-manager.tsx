...
export function BudgetsManager({ workshopId }: { workshopId: string }) {
  ...
  const parsePrice = (value: string) => Number(value.replace(".", "").replace(",", "."));
  ...
  function selectPriceItem(value: string) {
    if (!value) return;
    const [source, id] = value.split(":");
    const service = source === "catalog" ? catalog.find((item) => item.id === id) : undefined;
    const rate = source === "time" ? timeRates.find((item) => item.id === id) : undefined;
    if (service) { setKind("service"); setDescription(service.name); setPrice(String(service.minimum_price)); }
    if (rate) { setKind("service"); setDescription(rate.name); setPrice(String(rate.price)); }
  }
  ...
  return <div className="grid gap-8 xl:grid-cols-[400px_1fr]">
    ...
    {selected && <div className="mt-5 rounded-xl border border-zinc-800 bg-[#1A1A1A] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">Itens, serviços e valores</h3><PrintDocument type="Orçamento" number={selected.id.slice(0, 8).toUpperCase()} client={selected.clients?.full_name || "Cliente"} vehicle={`${selected.vehicles?.brand || ""} ${selected.vehicles?.model || ""}`} status={states.find((item) => item[0] === selected.status)?.[1] || selected.status} items={items} /></div>
      <select className="field mt-4" defaultValue="" onChange={(event) => selectPriceItem(event.target.value)}><option value="">Selecionar da precificação de mão de obra</option>{catalog.map((item) => <option key={item.id} value={`catalog:${item.id}`}>{item.category} · {item.name} ({priceText(item.minimum_price)} a {priceText(item.maximum_price)})</option>)}{timeRates.map((item) => <option key={item.id} value={`time:${item.id}`}>Tempo · {item.name} ({priceText(item.price)})</option>)}</select>
