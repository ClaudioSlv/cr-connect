...
export function BudgetsManager({ workshopId }: { workshopId: string }) {
  ...
  async function load() {
    const [clientsResult, vehiclesResult, budgetsResult, catalogResult, ratesResult] = await Promise.all([
      ...
      db.from("budgets").select("id,client_id,vehicle_id,service_order_id,status,clients(id,full_name,phone,whatsapp),vehicles(id,client_id,brand,model)").eq("workshop_id", workshopId).order("created_at", { ascending: false }),
  ...
  async function create(event: FormEvent) {
    ...
    const existing = await db.from("budgets").select("id,client_id,vehicle_id,service_order_id,status,clients(id,full_name,phone,whatsapp),vehicles(id,client_id,brand,model)").eq("workshop_id", workshopId).eq("client_id", client).eq("vehicle_id", vehicle).in("status", ["draft", "sent"]).limit(1).maybeSingle();
    ...
    const { data, error } = await db.from("budgets").insert({ workshop_id: workshopId, client_id: client, vehicle_id: vehicle, service_order_id: orderResult.data?.id || null, status: "draft" }).select("id,client_id,vehicle_id,service_order_id,status,clients(id,full_name,phone,whatsapp),vehicles(id,client_id,brand,model)").single();
  ...
  async function sendCardCheckoutByWhatsApp() {
    if (!selected || total <= 0) { setMessage("Adicione itens antes de enviar o pagamento por cartão."); return; }
    const phone = (selected.clients?.whatsapp || selected.clients?.phone || "").replace(/\D/g, "");
    if (!phone) { setMessage("Cadastre o WhatsApp ou telefone da cliente antes de enviar o cartão."); return; }
  ...
  return <div className="grid gap-8 xl:grid-cols-[400px_1fr]">
    ...
    {selected && <div className="mt-5 rounded-xl border border-zinc-800 bg-[#1A1A1A] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">Itens, serviços e valores</h3><PrintDocument type="Orçamento" number={selected.id.slice(0, 8).toUpperCase()} client={selected.clients?.full_name || "Cliente"} vehicle={`${selected.vehicles?.brand || ""} ${selected.vehicles?.model || ""}`} status={states.find((item) => item[0] === selected.status)?.[1] || selected.status} items={items} /></div>
      ...
      <p className="mt-5 text-right text-xl font-bold text-[#FFC107]">Total: {priceText(total)}</p>{selected.status === "draft" && <div className="mt-4 grid gap-3 sm:grid-cols-2"><button disabled={saving} type="button" onClick={() => void send()} className="rounded-lg bg-[#FFC107] px-4 py-3 font-bold text-black disabled:opacity-60">{saving ? "Aguarde…" : "Enviar para aprovação do cliente"}</button><button disabled={saving} type="button" onClick={() => void sendCardCheckoutByWhatsApp()} className="rounded-lg border border-zinc-500 bg-zinc-900 px-4 py-3 font-bold text-white disabled:opacity-60">{saving ? "Criando link…" : "∞ Cartão no WhatsApp"}</button></div>}<p className="mt-3 text-xs text-zinc-500">No cartão não há desconto. O Pix com 7% continua no PDF.</p></div>}</section></div>;
}
