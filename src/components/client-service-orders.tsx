...
export function ClientServiceOrders() {
  ...
  async function load() {
    ...
    setClient(current);
    if (!current) return;
    const [vehicleResult, orderResult, requestResult] = await Promise.all([
  ...
  async function send(event: FormEvent) {
    ...
    if (!result.error) { setCar(""); setDefect(""); await load(); }
  }

  if (!client) return <section className="mt-8 rounded-xl border border-[#4c3c1d] bg-[#171717] p-5"><p className="text-xs font-bold tracking-[.18em] text-[#FFC107]">ABRIR O.S.</p><h2 className="mt-2 text-xl font-bold">Vincule sua conta à oficina</h2><p className="mt-2 text-zinc-300">Peça para a oficina cadastrar você como <b>dono de veículo</b> usando exatamente este e-mail:</p><div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#6b510d] bg-[#211805] p-3"><b className="break-all text-[#FFC107]">{accountEmail || "Carregando e-mail..."}</b>{accountEmail && <button type="button" onClick={() => void navigator.clipboard.writeText(accountEmail)} className="rounded-lg border border-[#FFC107] px-3 py-2 text-sm font-bold text-[#FFC107]">Copiar e-mail</button>}</div><p className="mt-4 text-sm text-zinc-400">Depois de a oficina salvar o cadastro, toque em atualizar vínculo para abrir sua O.S.</p><button type="button" onClick={() => void load()} className="mt-4 rounded-lg bg-[#FFC107] px-4 py-2 font-bold text-black">Atualizar vínculo</button></section>;

  return <section className="mt-8"><p className="text-xs font-bold tracking-[.18em] text-[#FFC107]">PORTAL DO CLIENTE</p><h2 className="mt-2 text-2xl font-bold">Minhas Ordens de Serviço</h2><form onSubmit={send} className="mt-5 rounded-xl border border-[#FFC107] bg-[#171717] p-5"><h3 className="font-bold">Abrir solicitação de O.S.</h3><p className="mt-1 text-sm text-zinc-400">Escolha seu veículo e descreva o problema. A oficina receberá seu pedido.</p><select required value={car} onChange={(event) => setCar(event.target.value)} className="field mt-3"><option value="">Selecione o veículo</option>{cars.map((item) => <option key={item.id} value={item.id}>{item.brand} {item.model} {item.plate || ""}</option>)}</select><textarea required minLength={4} value={defect} onChange={(event) => setDefect(event.target.value)} className="field mt-3 min-h-24" placeholder="Descreva o defeito do carro"/><button className="mt-3 rounded-lg bg-[#FFC107] px-4 py-2 font-bold text-black">Enviar para oficina</button>{message && <p className="mt-3 text-sm text-[#FFC107]">{message}</p>}</form><div className="mt-7 flex items-end justify-between gap-3"><div><h3 className="text-xl font-bold">Status do andamento</h3><p className="mt-1 text-sm text-zinc-400">Verde: confirmado · Amarelo: aguardando · Vermelho: recusado ou cancelado.</p></div><button type="button" onClick={() => void load()} className="rounded-lg border border-zinc-600 px-3 py-2 text-sm font-bold">Atualizar</button></div><div className="mt-4 space-y-3">{requests.map((request) => <article key={request.id} className="rounded-xl border border-zinc-800 bg-[#171717] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><b>Solicitação enviada</b><span className={`rounded-full border px-3 py-1 text-sm font-bold ${request.status === "converted" ? "border-emerald-500/60 bg-emerald-500/10 text-emerald-300" : request.status === "declined" ? "border-red-500/60 bg-red-500/10 text-red-300" : "border-[#FFC107]/70 bg-[#FFC107]/10 text-[#FFC107]"}`}>{request.status === "converted" ? "Aceita pela oficina" : request.status === "declined" ? "Recusada pela oficina" : "Aguardando resposta da oficina"}</span></div><p className="mt-3 text-sm text-zinc-300">{request.complaint}</p></article>)}{orders.map((order) => { const presentation = statusStyle(order); return <article key={order.id} className="rounded-xl border border-zinc-800 bg-[#171717] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><b>O.S. #{order.number}</b><span className={`rounded-full border px-3 py-1 text-sm font-bold ${presentation.className}`}>{presentation.label}</span></div><p className="mt-3 text-sm text-zinc-300">{order.customer_complaint}</p></article>; })}{requests.length === 0 && orders.length === 0 && <p className="rounded-xl border border-zinc-800 bg-[#171717] p-5 text-zinc-400">Nenhuma solicitação ou O.S. encontrada.</p>}</div></section>;
}
