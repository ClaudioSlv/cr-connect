...
const labels: Record<string, string> = { open: "O.S. aberta", diagnosing: "Em diagnóstico", awaiting_approval: "Aguardando sua confirmação", awaiting_appointment: "Escolha a data da visita", appointment_confirmed: "Agendamento confirmado", awaiting_part: "Aguardando peça", repairing: "Em reparo", finished: "CARRO PRONTO PARA RETIRADA", delivered: "Entregue", cancelled: "Cancelada" };
...
function statusStyle(order: Order) {
  ...
  if (order.appointment_confirmed_at || ["appointment_confirmed", "finished", "delivered"].includes(order.status)) return { label: order.appointment_confirmed_at ? "Confirmado pela oficina" : labels[order.status], className: "border-emerald-500/60 bg-emerald-500/10 text-emerald-300" };
  if (order.appointment_requested_at) return { label: "Aguardando confirmação da oficina", className: "border-[#FFC107]/70 bg-[#FFC107]/10 text-[#FFC107]" };
...
export function ClientServiceOrders() {
  ...
  const [appointmentByOrder, setAppointmentByOrder] = useState<Record<string, string>>({});
