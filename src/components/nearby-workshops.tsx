...
type SentRequest = { workshop: Workshop; phone: string; requestId: string };
...
export function NearbyWorkshops() {
  ...
  function whats(item: Workshop) {
    ...
    return `https://wa.me/${number.startsWith("55") ? number : `55${number}`}?text=${encodeURIComponent(`Olá, preciso de ajuda pelo CR SOS. Serviço: ${service || item.emergency_services[0] || "emergência"}.${place}`)}`;
  }
  ...
  function locate() {
    ...
    navigator.geolocation.getCurrentPosition(async (result) => {
      ...
      setPosition(current); setItems(list); setMessage(list.length ? "Oficinas encontradas perto de você." : "Nenhuma oficina CR SOS ativa no seu raio agora."); setLoading(false);
  ...
  async function toggleReviews(workshop: Workshop) {
    ...
    if (!reviews[workshop.id]) {
      const { data } = await db.rpc("get_sos_workshop_reviews", { p_workshop_id: workshop.id });
      setReviews((current) => ({ ...current, [workshop.id]: (data || []) as Review[] }));
    }
    setReviewsOpen(workshop.id);
  }
