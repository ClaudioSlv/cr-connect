...
export function OnboardingForm() {
  ...
  if (!kind) return <div className="w-full max-w-2xl rounded-2xl border border-zinc-800 bg-[#1A1A1A] p-7"><p className="text-xs font-bold tracking-[.2em] text-[#FFC107]">PRIMEIRO ACESSO</p><h1 className="mt-3 text-3xl font-bold">Qual tipo de cadastro você quer criar?</h1><p className="mt-2 text-zinc-400">Escolha uma opção para continuar.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><button onClick={() => setKind("owner")} className="rounded-xl border border-[#FFC107] p-5 text-left"><b className="text-lg">Sou dono de veículo</b><span className="mt-2 block text-sm text-zinc-400">Grátis · cadastre seus veículos, acompanhe serviços e use o CR SOS.</span></button><button onClick={() => setKind("workshop")} className="rounded-xl border border-zinc-700 p-5 text-left"><b className="text-lg">Sou proprietário de oficina</b><span className="mt-2 block text-sm text-zinc-400">Crie o ambiente para administrar clientes, serviços e sua oficina.</span></button></div></div>;
